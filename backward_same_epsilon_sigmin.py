...
V = Vh_A.T[:,-1:]
V /= norm(V, ord=2)
sigma_min, sigma_max = np.min(s_A), np.max(s_A)
...
plt.tight_layout()
os.makedirs('rank1_sigmin', exist_ok=True)
plt.savefig('rank1_sigmin/'+'new_'+title_str+('_same_epsilon_small_update.pdf' if small_update else '_same_epsilon_large_update.pdf'))
plt.close()
