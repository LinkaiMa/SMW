...
def parse_args():
    ...
    parser.add_argument('--cond_scale', type=float, default=10, help='Condition number scale (default: 10)')
...
U_A, s_A, Vh_A = np.linalg.svd(np.random.normal(size=(n,n),scale= scale))
s_new = np.logspace(np.log10(s_A[0]), np.log10(s_A[0] / cond_scale), n)
A = U_A @ np.diag(s_new) @ Vh_A
U = np.random.normal(size=(n,k))
U /= norm(U, ord=2)
V = np.random.normal(size=(n,k))
V /= norm(V, ord=2)
sigma_min, sigma_max = np.min(s_new), np.max(s_new)
...
label_map = {
    ...
    'bound': r"$2 \kappa(A) \|A\|_2 \widetilde\epsilon_1 + 10.4 \widetilde\epsilon_2 $",
...
ax.set_ylabel("Value")
ax.set_title(f'Backward Error n = {n}, k = {k}, cond(A) = {cond_scale}, '+r'$\lambda$'+f' = {lamda_scale} '+(r'$\cdot\sigma_{\min}(A)$' if small_update else r'$\cdot\sigma_{\max}(A)$'))
...
plt.savefig('Figure3/'+'new_'+title_str+'_cond_'+str(cond_scale)+('_same_epsilon_small_update.pdf' if small_update else '_same_epsilon_large_update.pdf'))
plt.close()

print(f'cond1_idx: {cond1_idx}, cond2_idx: {cond2_idx}')